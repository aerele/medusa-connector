...
def _apply_inventory_update(
	...
	try:
		...
		# ---------------------------------------------------------
		# Get product title for inventory item naming.
		# ---------------------------------------------------------
		product_title = None
		...
		# ---------------------------------------------------------
		# Find all inventory items linked to this variant.
		# ---------------------------------------------------------
		...
		#
		# Fail safely. Do not guess which inventory item
		# should receive ERPNext stock.
		# ---------------------------------------------------------
		...
		# No inventory item.
		#
		# Create an inventory item directly with variant_id.
		# Medusa creates the relation as part of creation.
		# ---------------------------------------------------------
		if inventory_item_count == 0:
			...
			# -----------------------------------------------------
			# Re-fetch the variant after creation.
			#
			# This verifies that the new inventory item is actually
			# linked to the variant before we update stock.
			# -----------------------------------------------------
			updated_variant = (
				product_service.get_variant(
					d.variant_id,
		...
		#
		# Reuse it. Never create another one.
		# ---------------------------------------------------------
		else:
		...
		# ---------------------------------------------------------
		# Calculate ERPNext available stock.
		# ---------------------------------------------------------
		available_qty = max(cint(d.actual_qty) - cint(d.reserved_qty), 0)
		d.qty = available_qty

		# ---------------------------------------------------------
		# Create or update Medusa location level.
		# ---------------------------------------------------------
...
def _get_inventory_levels(warehouse_map: dict[str, str]) -> list:
	"""Get changed ERPNext inventory levels."""
	levels = []
	for warehouse in warehouse_map:
		if not warehouse:
			continue

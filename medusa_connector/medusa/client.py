...
class MedusaClient:
	...
	def _request(self, method: str, url: str, **kwargs) -> dict:
		...
		Automatically retries transient network failures using exponential
		...
		for attempt in range(MAX_RETRIES):
			try:
				...
				if attempt == MAX_RETRIES - 1:
					frappe.log_error(
						f"Medusa API request failed after {MAX_RETRIES} attempts.\n"
						f"Method: {method}\n"
						f"URL: {url}\n"
						f"Error: {exc}",
						"Medusa API Request Failed",
					)
					raise
	...
	def list_webhooks(self, page_limit: int = DEFAULT_PAGE_LIMIT) -> list[dict]:
		"""Return all webhook subscriptions registered in Medusa.

		Args:
		    page_limit: Number of items to fetch per page (default: DEFAULT_PAGE_LIMIT)

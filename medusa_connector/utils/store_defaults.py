...
def refresh_store_defaults(client=None, *, commit: bool = False) -> dict:
	...
	_persist_defaults(defaults, commit=commit)
...
def get_store_defaults(*, refresh_if_empty: bool = True) -> dict:
	...
	if refresh_if_empty and settings.enabled and not defaults["default_currency"]:
		try:
			defaults = refresh_store_defaults(commit=True)
		except Exception:
			frappe.log_error(
...
def _persist_defaults(defaults: dict, *, commit: bool) -> None:

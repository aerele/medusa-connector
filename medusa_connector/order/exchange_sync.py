...
class ExchangeSync:
	...
	def process(
		...
		if exchange_data.get("items"):
			return_dn_name = self._create_exchange_return_delivery_note(order, exchange_data, so)
	...
	def _resolve_order_and_exchange(
		...
		if not oid:
			# Try to get order from exchange endpoint
	...
	def _create_exchange_new_delivery_note(self, order: dict, exchange_data: dict, sales_order) -> str | None:
		...
		# Process new items
		new_items = self._get_exchange_new_items(dn.items, additional_items, order, warehouse)
		if not new_items:
			return None

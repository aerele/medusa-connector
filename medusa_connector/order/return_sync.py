...
class ReturnSync:
	...
	def process(
		...
		# A return only actually moves stock once the warehouse has marked it
		# "received" — not merely "requested". Both order.return_requested and
		# order.return_received land here, but we always re-fetch the return's
		# *current* status from Medusa (above), so whichever webhook fired,
		# the Delivery Note is only created once the return is truly received.
		# This also means the refund step (RefundSync) can rely on "a
		# Delivery Note exists for this return" to mean "it was received".
		if rstatus not in ("received", "partially_received"):
			frappe.db.set_value(
			...
			return result(
				...
				sales_order=so.name,
				message=_(
					"Return {0} is '{1}'; Delivery Note will be created once the return is received."
				).format(return_id, rstatus),
			)
	...
	def _resolve_order_and_return(return_id: str, order_id: str | None) -> tuple[dict | None, dict | None]:
		...
		if not oid:
			entity = OrderService().get_return(rid)
	...
	def _create_return_delivery_note(self, order: dict, return_data: dict, sales_order) -> str | None:
		...
		# Create a return against a submitted outbound Delivery Note.
		source_dn = frappe.db.get_value(
			"Delivery Note", {ORDER_ID_FIELD: order_id, "is_return": 0, "docstatus": 1}, "name"
		)
		if not source_dn:
			return None
		dn = make_return_doc("Delivery Note", source_dn)
		...
		warehouse = self.settings.get("return_warehouse") or self.settings.get("warehouse")
	...
	def _get_return_items(dn_items, return_data: dict, order: dict, warehouse: str) -> list:
		...
		for rline in return_data.get("items") or []:
			...
			qty = flt(rline.get("received_quantity") or rline.get("quantity") or 0)
				...
				resolved.append(
					{
						"item_code": item_code,
						"qty": qty,
						"damaged_qty": flt(rline.get("damaged_quantity") or 0),
					}
				)
		...
		for dn_item in dn_items:
			...
			target_warehouse = (
				frappe.get_single(SETTING_DOCTYPE).get("damaged_return_warehouse")
				if match.get("damaged_qty")
				else warehouse
			)
			if not target_warehouse:
				frappe.throw(
					_(
						"Configure Return Warehouse and Damaged Return Warehouse before receiving Medusa returns."
					)
				)
			update = {"qty": -abs(match["qty"]), "warehouse": target_warehouse}

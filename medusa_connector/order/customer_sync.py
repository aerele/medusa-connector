# Copyright (c) 2026, Aerele Technologies and contributors
# For license information, please see license.txt
"""Customer and address synchronization for Medusa orders."""

from __future__ import annotations

from typing import Any

import frappe
from ecommerce_core.controllers.customer import EcommerceCustomer
from ecommerce_core.utils.address_mapping import get_country_name
from frappe import _
from frappe.utils import cstr, validate_phone_number
from frappe.utils.nestedset import get_root_of

from medusa_connector.constants import (
	ADDRESS_ID_FIELD,
	CUSTOMER_ID_FIELD,
	MODULE_NAME,
	SETTING_DOCTYPE,
)
from medusa_connector.medusa.customer import CustomerService


class CustomerSync:
	"""Resolve and synchronize the Medusa customer and its addresses."""

	def __init__(self, settings=None):
		self.settings = settings or frappe.get_cached_doc(SETTING_DOCTYPE)

	def sync(self, order: dict) -> dict[str, str | None]:
		"""Create/update customer and addresses for the Medusa order.

		Returns ERPNext Customer and Billing/Shipping Address names.
		"""
		return resolve_order_customer(order, settings=self.settings)


class MedusaCustomer(EcommerceCustomer):
	"""Customer adapter for Medusa order payloads."""

	def __init__(self, customer_id: str, settings=None):
		self.setting = settings or frappe.get_cached_doc(SETTING_DOCTYPE)
		super().__init__(cstr(customer_id), CUSTOMER_ID_FIELD, MODULE_NAME)

	def sync_customer(self, customer: dict[str, Any]) -> None:
		"""Create ERPNext Customer, addresses, and contact."""
		customer_name = _customer_display_name(customer)
		customer_group = self._get_customer_group()

		super().sync_customer(customer_name, customer_group)

		self._sync_address(
			customer_name,
			self._get_billing_address(customer),
			"Billing",
			customer.get("email"),
		)

		self._sync_address(
			customer_name,
			self._get_shipping_address(customer),
			"Shipping",
			customer.get("email"),
		)

		self.create_customer_contact(customer)

	def update_customer_context(self, customer: dict[str, Any]) -> None:
		"""Update existing customer's billing/shipping addresses."""
		customer_name = self.get_customer_doc().name
		email = customer.get("email")

		self._sync_address(
			customer_name,
			self._get_billing_address(customer),
			"Billing",
			email,
		)

		self._sync_address(
			customer_name,
			self._get_shipping_address(customer),
			"Shipping",
			email,
		)

	def get_address_names(self) -> dict[str, str | None]:
		"""Return existing ERPNext Billing and Shipping Address names."""
		customer_name = self.get_customer_doc().name

		return {
			"billing_address": self._get_existing_address_name(customer_name, "Billing"),
			"shipping_address": self._get_existing_address_name(customer_name, "Shipping"),
		}

	def _sync_address(
		self,
		customer_name: str,
		medusa_address: dict[str, Any] | None,
		address_type: str,
		email: str | None,
	) -> None:
		if not medusa_address:
			return

		old_address = self.get_customer_address_doc(address_type)

		if not old_address:
			self.create_customer_address(
				customer_name,
				medusa_address,
				address_type,
				email,
			)
			return

		new_values = _map_address_fields(
			medusa_address,
			customer_name,
			address_type,
			email,
		)

		# Keep ERPNext address identity fields unchanged.
		for field in ("address_title", "address_type"):
			new_values.pop(field, None)

		old_address.update(new_values)
		old_address.flags.ignore_mandatory = True
		old_address.save()

	def create_customer_address(
		self,
		customer_name: str,
		medusa_address: dict[str, Any],
		address_type: str = "Billing",
		email: str | None = None,
	) -> None:
		address_fields = _map_address_fields(
			medusa_address,
			customer_name,
			address_type,
			email,
		)
		super().create_customer_address(address_fields)

	def create_customer_contact(
		self,
		medusa_customer: dict[str, Any],
	) -> None:
		first = cstr(medusa_customer.get("first_name")).strip()
		email = cstr(medusa_customer.get("email")).strip()

		if not (first or email):
			return

		if not first:
			first = email.split("@")[0]

		contact_fields: dict[str, Any] = {
			"status": "Passive",
			"first_name": first,
			"last_name": cstr(medusa_customer.get("last_name")).strip() or None,
		}

		if email:
			contact_fields["email_ids"] = [
				{
					"email_id": email,
					"is_primary": True,
				}
			]

		billing_address = medusa_customer.get("billing_address") or {}
		phone_no = medusa_customer.get("phone") or billing_address.get("phone")

		if phone_no and validate_phone_number(
			cstr(phone_no),
			throw=False,
		):
			contact_fields["phone_nos"] = [
				{
					"phone": phone_no,
					"is_primary_phone": True,
				}
			]

		super().create_customer_contact(contact_fields)

	def _get_customer_group(self) -> str | None:
		"""Get customer group from settings. No fallback - returns None if not configured."""
		customer_group = self.setting.customer_group
		if customer_group and frappe.db.exists("Customer Group", customer_group):
			return customer_group
		return None

	@staticmethod
	def _get_billing_address(
		customer: dict[str, Any],
	) -> dict[str, Any] | None:
		return (
			customer.get("billing_address")
			or customer.get("default_billing_address")
			or _default_profile_address(customer, billing=True)
		)

	@staticmethod
	def _get_shipping_address(
		customer: dict[str, Any],
	) -> dict[str, Any] | None:
		return (
			customer.get("shipping_address")
			or customer.get("default_shipping_address")
			or _default_profile_address(customer, billing=False)
		)

	@staticmethod
	def _get_existing_address_name(
		customer_name: str,
		address_type: str,
	) -> str | None:
		rows = frappe.get_all(
			"Dynamic Link",
			filters={
				"link_doctype": "Customer",
				"link_name": customer_name,
				"parenttype": "Address",
			},
			fields=["parent"],
		)

		for row in rows:
			if (
				frappe.db.get_value(
					"Address",
					row.parent,
					"address_type",
				)
				== address_type
			):
				return row.parent

		return rows[0].parent if rows else None


def resolve_order_customer(
	order: dict,
	*,
	settings=None,
	service: CustomerService | None = None,
) -> dict[str, str | None]:
	"""Resolve and synchronize Customer + Billing/Shipping Addresses."""
	settings = settings or frappe.get_cached_doc(SETTING_DOCTYPE)

	if not order:
		customer = _default_customer_or_throw(settings)
		return {
			"customer": customer,
			"billing_address": None,
			"shipping_address": None,
		}

	customer_id = cstr(order.get("customer_id") or (order.get("customer") or {}).get("id") or "")

	if not customer_id:
		customer = _default_customer_or_throw(settings)
		return {
			"customer": customer,
			"billing_address": None,
			"shipping_address": None,
		}

	payload = _payload_from_order(
		order,
		customer_id=customer_id,
	)

	if not _has_customer_details(payload):
		service = service or CustomerService()

		try:
			fetched = service.get_customer(customer_id)

			if fetched:
				payload = _merge_customer_payload(
					fetched,
					order,
				)
				payload["id"] = customer_id

		except Exception as exc:
			frappe.logger("medusa_connector").warning(
				f"Could not hydrate Medusa customer {customer_id} for order: {exc}"
			)

	medusa_customer = MedusaCustomer(
		customer_id,
		settings=settings,
	)

	if not medusa_customer.is_synced():
		medusa_customer.sync_customer(payload)
	else:
		medusa_customer.update_customer_context(payload)

	addresses = medusa_customer.get_address_names()

	return {
		"customer": medusa_customer.get_customer_doc().name,
		"billing_address": addresses["billing_address"],
		"shipping_address": addresses["shipping_address"],
	}


def _payload_from_order(
	order: dict,
	*,
	customer_id: str,
) -> dict[str, Any]:
	payload: dict[str, Any] = {
		"id": customer_id,
	}

	nested = order.get("customer") if isinstance(order.get("customer"), dict) else {}

	payload.update({key: value for key, value in nested.items() if value is not None})

	if order.get("email") and not payload.get("email"):
		payload["email"] = order["email"]

	if order.get("billing_address"):
		payload["billing_address"] = order["billing_address"]

	if order.get("shipping_address"):
		payload["shipping_address"] = order["shipping_address"]

	return payload


def _merge_customer_payload(
	customer: dict,
	order: dict | None = None,
) -> dict[str, Any]:
	payload = dict(customer or {})

	if order:
		if order.get("billing_address") and not payload.get("billing_address"):
			payload["billing_address"] = order["billing_address"]

		if order.get("shipping_address") and not payload.get("shipping_address"):
			payload["shipping_address"] = order["shipping_address"]

		if order.get("email") and not payload.get("email"):
			payload["email"] = order["email"]

	return payload


def _has_customer_details(payload: dict) -> bool:
	return bool(
		payload.get("email")
		or payload.get("first_name")
		or payload.get("last_name")
		or payload.get("company_name")
		or payload.get("company")
		or payload.get("addresses")
		or payload.get("billing_address")
		or payload.get("shipping_address")
	)


def _default_profile_address(
	customer: dict,
	*,
	billing: bool,
) -> dict | None:
	addresses = customer.get("addresses") or []

	if not isinstance(addresses, list):
		return None

	for address in addresses:
		if not isinstance(address, dict):
			continue

		if billing and address.get("is_default_billing"):
			return address

		if not billing and address.get("is_default_shipping"):
			return address

	return addresses[0] if addresses else None


def _customer_display_name(payload: dict) -> str:
	company = cstr(payload.get("company_name") or payload.get("company")).strip()

	if company:
		return company[:140]

	first = cstr(payload.get("first_name")).strip()

	last = cstr(payload.get("last_name")).strip()

	name = f"{first} {last}".strip()

	if name:
		return name[:140]

	email = cstr(payload.get("email")).strip()

	if email:
		return email[:140]

	return cstr(payload.get("id") or "Medusa Customer")[:140]


def _default_customer_or_throw(settings) -> str:
	default = settings.get("default_customer")

	if not default:
		frappe.throw(
			_("Default Customer is not set on Medusa Settings. Set a Default Customer for guest checkouts."),
			title=_("Customer Required"),
		)

	return default


def _map_address_fields(
	medusa_address: dict,
	customer_name: str,
	address_type: str,
	email: str | None,
) -> dict[str, Any]:
	"""Map Medusa address to ERPNext Address fields."""
	country_code = medusa_address.get("country_code") or medusa_address.get("country")

	country = get_country_name(cstr(country_code).upper() if country_code else None)

	if (
		not country
		and country_code
		and frappe.db.exists(
			"Country",
			cstr(country_code),
		)
	):
		country = cstr(country_code)

	if not country:
		country = frappe.db.get_default("country")

	line1 = (
		medusa_address.get("address_1")
		or medusa_address.get("address_line1")
		or medusa_address.get("address1")
		or "Address 1"
	)

	line2 = (
		medusa_address.get("address_2")
		or medusa_address.get("address_line2")
		or medusa_address.get("address2")
	)

	fields: dict[str, Any] = {
		"address_title": customer_name,
		"address_type": address_type,
		"address_line1": cstr(line1)[:140],
		"address_line2": (cstr(line2)[:140] if line2 else None),
		"city": cstr(medusa_address.get("city") or "Unknown")[:100],
		"state": cstr(medusa_address.get("province") or medusa_address.get("state") or "")[:100],
		"pincode": cstr(
			medusa_address.get("postal_code")
			or medusa_address.get("zip")
			or medusa_address.get("pincode")
			or ""
		)[:20],
		"country": country,
		"email_id": (email or medusa_address.get("email")),
	}

	if medusa_address.get("id"):
		fields[ADDRESS_ID_FIELD] = cstr(medusa_address["id"])

	phone = medusa_address.get("phone")

	if phone and validate_phone_number(
		cstr(phone),
		throw=False,
	):
		fields["phone"] = cstr(phone)[:30]

	return fields

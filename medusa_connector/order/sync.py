...
class OrderSync:
	...
	def update_status_fields(self, order: dict) -> None:
		...
		Skips cancelled documents to prevent overwriting the cancelled status."""
		...
		for doctype in ("Sales Order", "Sales Invoice", "Delivery Note"):
			dt = DocType(doctype)
			# Skip cancelled documents (docstatus=2) to prevent overwriting cancelled status
			(
				frappe.qb.update(dt)
				.set(dt[ORDER_STATUS_FIELD], label)
				.where(dt[ORDER_ID_FIELD] == order_id)
	...
	def sync_old_orders(self) -> dict:
		...
		if not from_date or not to_date:
			...
			return {"synced": 0, "created": 0, "failed": 0, "status": "Error"}
	...
	def _sync(self, order: dict, request_id: str | None = None) -> dict[str, Any]:
		...
		if not self.settings.enabled:
			return result(
				MedusaOperationStatus.ERROR, message=_("Medusa Connector is disabled; order not synced.")
			)

		order_id = cstr(order.get("id") or "")
		if not order_id:
			return result(MedusaOperationStatus.ERROR, message=_("Medusa order payload has no id."))
		...
		if existing:
			...
			self.sales_order._validate_medusa_total(so, order)
			created = False
		else:
	...
	def _apply_lifecycle(self, order: dict, sales_order) -> dict[str, Any]:
		...
		for row in order.get("returns") or []:
			if isinstance(row, dict) and row.get("id"):
				lifecycle["returns"].append(
					ReturnSync(self.settings).process(return_id=cstr(row["id"]), order_id=order_id)
				)
		for row in order.get("claims") or []:
			if isinstance(row, dict) and row.get("id"):
				lifecycle["claims"].append(
					ClaimSync(self.settings).process(claim_id=cstr(row["id"]), order_id=order_id)
				)
		for row in order.get("exchanges") or []:
			if isinstance(row, dict) and row.get("id"):
				lifecycle["exchanges"].append(
					ExchangeSync(self.settings).process(exchange_id=cstr(row["id"]), order_id=order_id)
				)
		...
		if self.payment.is_refunded(order):
			...
			for collection in order.get("payment_collections") or []:
				for payment in collection.get("payments") or []:
					if payment.get("refunds") and payment.get("id"):
						lifecycle["refunds"].append(
							RefundSync(self.settings).process(payment_id=cstr(payment["id"]))
						)

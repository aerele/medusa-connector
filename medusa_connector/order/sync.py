...
def sync_sales_order(order: dict, request_id: str | None = None) -> str | None:
	...
	try:
		...
		if errors:
			message += _(" Errors: {0}").format("; ".join(errors))
...
def cancel_order(order: dict, request_id: str | None = None) -> str | None:
	...
	try:
		...
		if cstr(order.get("payment_status")) == "refunded":
...
def sync_old_orders() -> dict | None:
	...
	try:
		from_date = settings.old_orders_from
		to_date = settings.old_orders_to
		...
		return _run_old_orders_sync(from_date, to_date)

	except Exception:
		frappe.log_error(
			title="Sync old orders failed",
...
def _clear_sync_old_orders_flag() -> None:
	"""Disable the Sync Old Orders flag after a sync run."""
...
def update_order_status_fields(order: dict) -> None:
	...
	(
		...
		.set(sales_order[ORDER_STATUS_FIELD], label)
		.where(sales_order[ORDER_ID_FIELD] == order_id)
	).run()

	(
		frappe.qb.update(sales_invoice)
		.set(sales_invoice[ORDER_STATUS_FIELD], label)
		.where(sales_invoice[ORDER_ID_FIELD] == order_id)
	).run()

	(
		frappe.qb.update(delivery_note)
		.set(delivery_note[ORDER_STATUS_FIELD], label)
		.where(delivery_note[ORDER_ID_FIELD] == order_id)
	).run()

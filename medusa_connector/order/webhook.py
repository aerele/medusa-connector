...
class FulfillmentHandler:
	def handle(self, event: MedusaEvent) -> dict:
		...
		res = res or result(MedusaOperationStatus.ERROR, message=f"{event.name}: fulfillment sync failed")
...
@register("shipment.created", "delivery.created")
...
class ReturnHandler:
	def handle(self, event: MedusaEvent) -> dict:
		...
		order_id = cstr(entity.get("order_id") or "")
		if not return_id:
			return result(MedusaOperationStatus.ERROR, message=f"{event.name}: missing return id")
		return ReturnSync().process(return_id=return_id, order_id=order_id or None, request_id=event.log_name)
...
class ClaimHandler:
	def handle(self, event: MedusaEvent) -> dict:
		...
		order_id = cstr(entity.get("order_id") or "")
		if not claim_id:
			return result(MedusaOperationStatus.ERROR, message=f"{event.name}: missing claim id")
		return ClaimSync().process(claim_id=claim_id, order_id=order_id or None, request_id=event.log_name)
...
class ExchangeHandler:
	def handle(self, event: MedusaEvent) -> dict:
		...
		order_id = cstr(entity.get("order_id") or "")
		if not exchange_id:
			return result(MedusaOperationStatus.ERROR, message=f"{event.name}: missing exchange id")
		return ExchangeSync().process(
			exchange_id=exchange_id, order_id=order_id or None, request_id=event.log_name
		)
...
class PaymentHandler:
	...
	def _handle_refund(self, event: MedusaEvent, payment_id: str) -> dict:
		...
		res = res or result(MedusaOperationStatus.ERROR, message=f"{event.name}: refund sync failed")

...
class FulfillmentSync:
	...
	# -- public API (each is ONE log row) --------------------------------
	...
	def sync(
		...
		if not cint(self.settings.get("sync_delivery_note")):
			return result("skipped", message=_("Sync Delivery Note is disabled on Medusa Settings"))
	...
	def _cancel_impl(self, *, fulfillment_id: str, order_id: str | None = None) -> dict[str, Any]:
		...
		if not fulfillment_id:
			return result("invalid", message=_("Missing fulfillment_id"))

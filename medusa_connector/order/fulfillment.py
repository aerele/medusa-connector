...
def cancel_order_delivery_notes(
	...
	# Bulk update status field once instead of per Delivery Note.
	if status_label:
		(
			frappe.qb.update(delivery_note)
			.set(delivery_note[ORDER_STATUS_FIELD], status_label)
	...
	for dn in dns:
		...
		try:
			if dn.docstatus == 1:
				frappe.get_doc("Delivery Note", dn_name).cancel()
...
def _warehouse_for_fulfillment(fulfillment: dict, settings) -> str | None:
	...
	if location_id and hasattr(settings, "get_medusa_to_erpnext_wh_mapping"):
		wh_map = settings.get_medusa_to_erpnext_wh_mapping() or {}

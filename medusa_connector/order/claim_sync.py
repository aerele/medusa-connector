...
class ClaimSync:
	...
	def process(
		...
		if claim_type == "replace":
			...
			dn_name = self._create_claim_return_delivery_note(order, claim_data, so)
			result_data = result(
	...
	def _resolve_order_and_claim(claim_id: str, order_id: str | None) -> tuple[dict | None, dict | None]:
		...
		if not oid:
			# Try to get order from claim data
	...
	def _reverse_claim_payment(self, order: dict, claim_data: dict, sales_order) -> dict:
		"""Reverse payment and invoice for refund claims."""
		claim_id = cstr(claim_data.get("id") or "")
		order_id = cstr(order.get("id") or "")

		# Find existing SI for the order
		...
		if not si_name:
			return {
				...
				"sales_invoice": None,
				"payment_entry": None,
				"message": _("No submitted Sales Invoice for refund claim {0}").format(claim_id),
			}

		# Find payment entry
		pe_name = self._find_payment_entry(order_id, si_name)

		# Stamp claim ID
		if frappe.get_meta("Sales Invoice").has_field(CLAIM_ID_FIELD):
			frappe.db.set_value("Sales Invoice", si_name, CLAIM_ID_FIELD, claim_id, update_modified=False)
		if pe_name and frappe.get_meta("Payment Entry").has_field(CLAIM_ID_FIELD):
			frappe.db.set_value("Payment Entry", pe_name, CLAIM_ID_FIELD, claim_id, update_modified=False)

		# Cancel payment and invoice
		from medusa_connector.order._shared import cancel_doc
		...
		return {
			...
			"message": _("Claim {0}: Reversed Sales Invoice {1} and Payment Entry {2}").format(
				claim_id, si_name, pe_name or "-"
			),
		}
